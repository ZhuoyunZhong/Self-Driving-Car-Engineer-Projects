 ...
 * @param veh_x particle x
 * @param veh_y particle y
 * @param veh_theta particle theta
 */
inline std::vector<LandmarkObs> transform_coord(std::vector<LandmarkObs> observations,
                                         double veh_x, double veh_y, double veh_theta) {
  ...
  for (int i = 0; i < observations.size(); ++i) {
    ...
    trans_observations[i].x = veh_x + cos(veh_theta) * obs_x - sin(veh_theta) * obs_y;
    trans_observations[i].y = veh_y + sin(veh_theta) * obs_x + cos(veh_theta) * obs_y;
  }
